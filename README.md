...
TODO:
- Storage file, switch from json to plain text. Easier to append.
- Storage file, support existing file, should be correctly appended.
- Notify existing file is used and will be appended.
- If wrong format show error, do not continue.
- Close file when stopping (currently left open).
- Support reading existing items from progress file(same or other). Skip those records.
- Save load whole state, including new checkboxes and file paths.